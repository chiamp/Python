...
def score(string, goal):
    ...
    for char in string:
        if char in temp_dict:
            points += 1 #+1 points if the character in string is present in goal (represented by temp_dict)
            temp_dict[char] += -1
            if temp_dict[char] == 0:
...
def evolution(goal, num_per_gen, top, mutation_rate, mutation_extent):
    ...
    for string in mutate_list:
        if len(highest_scores) == 0: #if highest_scores is empty
            ...
            #    temp_list.append(score(i, 'marcus'))
            #print(temp_list)
            #print(string)
            #print(score(string, goal))
            #print(index)
            #if index < top:
                #print(highest_scores[index])
                #print(score(highest_scores[index], goal))
...
#        former code that kept parents into next generation if their fitness was better than their offspring
#        for string in mutate_list:
#            index = 0
#            while index < top and score(string, goal) < score(highest_scores[index], goal):
#                index += 1
#            if index < top and score(string, goal) > score(highest_scores[index], goal):
#                highest_scores.insert(index, string)
#                highest_scores.pop()
